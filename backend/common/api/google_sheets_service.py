...
class GoogleSheetsService:
    def __init__(self):
        ...
        if os.path.exists(GOOGLE_SHEETS_TOKEN_PATH):
            with open(GOOGLE_SHEETS_TOKEN_PATH, "rb") as token:
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    GOOGLE_SHEETS_CREDS_PATH, GOOGLE_SHEETS_SCOPES
                )
                creds = flow.run_local_server(port=0)
            with open(GOOGLE_SHEETS_TOKEN_PATH, "wb") as token:
